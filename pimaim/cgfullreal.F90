...
DOUBLE PRECISION :: exxquadi,eyyquadi,ezzquadi,exyquadi,exzquadi,eyzquadi, &
...
!    With PBC's corrections (AGUADO)
!
      exxquadi=-efact5*(chgquad2ij*qjrec)*dxx+ &
                efact3*(3.0d0*dxx*quadxx(i)+ &
                ((dxx+dyy)/2.0d0)*quadyy(i)+((dxx+dzz)/2.0d0)*quadzz(i)+ &
                3.0d0*dxy*quadxy(i)+3.0d0*dxz*quadxz(i)+dyz*quadyz(i))+ &
                2.0d0*efact3*(dxx*quadxx(i)+dxy*quadxy(i)+dxz*quadxz(i))- &
                2.0d0*efact2*quadxx(i)+ &
                efact6*((chgquad2ij*qjrec)*txx+ &
                    (chgquadengij*qjrec)*dxx)/2.0d0- &
                efact4*(dx*txijdotthetaji+ &
                    quadxx(i)*dx*txxx+quadyy(i)*dy*txxy+ &
                    quadzz(i)*dz*txxz+quadxy(i)*(dx*txxy+dy*txxx)+ &
                    quadxz(i)*(dx*txxz+dz*txxx)+quadyz(i)*(dy*txxz+dz*txxy))- &
                efact*(txxxx*quadxx(i)+ &
                2.0d0*txxxy*quadxy(i)+2.0d0*txxxz*quadxz(i)+ &
                txxyy*quadyy(i)+txxzz*quadzz(i)+2.0d0*txxyz*quadyz(i))
      eyyquadi=-efact5*(chgquad2ij*qjrec)*dyy+ &
                efact3*(3.0d0*dyy*quadyy(i)+ &
                ((dxx+dyy)/2.0d0)*quadxx(i)+((dyy+dzz)/2.0d0)*quadzz(i)+ &
                3.0d0*dxy*quadxy(i)+3.0d0*dyz*quadyz(i)+dxz*quadxz(i))+ &
                2.0d0*efact3*(dyy*quadyy(i)+dxy*quadxy(i)+dyz*quadyz(i))- &
                2.0d0*efact2*quadyy(i)+ &
                efact6*((chgquad2ij*qjrec)*tyy+ &
                        (chgquadengij*qjrec)*dyy)/2.0d0- &
                efact4*(dy*tyijdotthetaji+ &
                        quadxx(i)*dx*txyy+quadyy(i)*dy*tyyy+ &
                        quadzz(i)*dz*tyyz+quadxy(i)*(dx*tyyy+dy*txyy)+ &
                        quadxz(i)*(dx*tyyz+dz*txyy)+ &
                        quadyz(i)*(dy*tyyz+dz*tyyy))- &
                efact*(txxyy*quadxx(i)+ &
                2.0d0*txyyy*quadxy(i)+2.0d0*txyyz*quadxz(i)+ &
                tyyyy*quadyy(i)+tyyzz*quadzz(i)+2.0d0*tyyyz*quadyz(i))
      ezzquadi=-efact5*(chgquad2ij*qjrec)*dzz+ &
                efact3*(3.0d0*dzz*quadzz(i)+ &
                ((dxx+dzz)/2.0d0)*quadxx(i)+((dyy+dzz)/2.0d0)*quadyy(i)+ &
                3.0d0*dxz*quadxz(i)+3.0d0*dyz*quadyz(i)+dxy*quadxy(i))+ &
                2.0d0*efact3*(dzz*quadzz(i)+dxz*quadxz(i)+dyz*quadyz(i))- &
                2.0d0*efact2*quadzz(i)+ &
                efact6*((chgquad2ij*qjrec)*tzz+ &
                        (chgquadengij*qjrec)*dzz)/2.0d0- &
                efact4*(dz*tzijdotthetaji+ &
                        quadxx(i)*dx*txzz+quadyy(i)*dy*tyzz+ &
                        quadzz(i)*dz*tzzz+quadxy(i)*(dx*tyzz+dy*txzz)+ &
                        quadxz(i)*(dx*tzzz+dz*txzz)+ &
                        quadyz(i)*(dy*tzzz+dz*tyzz))- &
                efact*(txxzz*quadxx(i)+ &
                2.0d0*txyzz*quadxy(i)+2.0d0*txzzz*quadxz(i)+ &
                tyyzz*quadyy(i)+tzzzz*quadzz(i)+2.0d0*tyzzz*quadyz(i))
      exyquadi=-efact5*(chgquad2ij*qjrec)*dxy+ &
                efact3*((3.0d0*dxy/2.0d0)*(quadxx(i)+quadyy(i))+ &
                (dxy/2.0d0)*quadzz(i)+(dxx+dyy)*quadxy(i)+ &
                dxz*quadyz(i)+dyz*quadxz(i))+ &
                efact3*((dxx+dyy)*quadxy(i)+ &
                     dxy*(quadxx(i)+quadyy(i))+dxz*quadyz(i)+dyz*quadxz(i))- &
                2.0d0*efact2*quadxy(i)+ &
                efact6*((chgquad2ij*qjrec)*txy+ &
                        (chgquadengij*qjrec)*dxy)/2.0d0- &
                efact4*((dx*tyijdotthetaji+ &
                         dy*txijdotthetaji)/2.0d0+ &
                         quadxx(i)*dx*txxy+quadyy(i)*dy*txyy+ &
                         quadzz(i)*dz*txyz+quadxy(i)*(dx*txyy+dy*txxy)+ &
                         quadxz(i)*(dx*txyz+dz*txxy)+ &
                         quadyz(i)*(dy*txyz+dz*txyy))- &
                efact*(txxxy*quadxx(i)+ &
                2.0d0*txxyy*quadxy(i)+2.0d0*txxyz*quadxz(i)+ &
                txyyy*quadyy(i)+txyzz*quadzz(i)+2.0d0*txyyz*quadyz(i))
      exzquadi=-efact5*(chgquad2ij*qjrec)*dxz+ &
                efact3*((3.0d0*dxz/2.0d0)*(quadxx(i)+quadzz(i))+ &
                (dxz/2.0d0)*quadyy(i)+(dxx+dzz)*quadxz(i)+ &
                dxy*quadyz(i)+dyz*quadxy(i))+ &
                efact3*((dxx+dzz)*quadxz(i)+ &
                    dxz*(quadxx(i)+quadzz(i))+dxy*quadyz(i)+dyz*quadxy(i))- &
                2.0d0*efact2*quadxz(i)+ &
                efact6*((chgquad2ij*qjrec)*txz+ &
                        (chgquadengij*qjrec)*dxz)/2.0d0- &
                efact4*((dx*tzijdotthetaji+ &
                         dz*txijdotthetaji)/2.0d0+ &
                        quadxx(i)*dx*txxz+quadyy(i)*dy*txyz+ &
                        quadzz(i)*dz*txzz+quadxy(i)*(dx*txyz+dy*txxz)+ &
                        quadxz(i)*(dx*txzz+dz*txxz)+ &
                        quadyz(i)*(dy*txzz+dz*txyz))- &
                efact*(txxxz*quadxx(i)+ &
                2.0d0*txxyz*quadxy(i)+2.0d0*txxzz*quadxz(i)+ &
                txyyz*quadyy(i)+txzzz*quadzz(i)+2.0d0*txyzz*quadyz(i))
      eyzquadi=-efact5*(chgquad2ij*qjrec)*dyz+ &
                efact3*((3.0d0*dyz/2.0d0)*(quadyy(i)+quadzz(i))+ &
                (dyz/2.0d0)*quadxx(i)+(dyy+dzz)*quadyz(i)+ &
                dxy*quadxz(i)+dxz*quadxy(i))+ &
                efact3*((dyy+dzz)*quadyz(i)+ &
                    dyz*(quadyy(i)+quadzz(i))+dxy*quadxz(i)+dxz*quadxy(i))- &
                2.0d0*efact2*quadyz(i)+ &
                efact6*((chgquad2ij*qjrec)*tyz+ &
                        (chgquadengij*qjrec)*dyz)/2.0d0- &
                efact4*((dy*tzijdotthetaji+ &
                         dz*tyijdotthetaji)/2.0d0+ &
                        quadxx(i)*dx*txyz+quadyy(i)*dy*tyyz+ &
                        quadzz(i)*dz*tyzz+quadxy(i)*(dx*tyyz+dy*txyz)+ &
                        quadxz(i)*(dx*tyzz+dz*txyz)+ &
                        quadyz(i)*(dy*tyzz+dz*tyyz))- &
                efact*(txxyz*quadxx(i)+ &
                2.0d0*txyyz*quadxy(i)+2.0d0*txyzz*quadxz(i)+ &
                tyyyz*quadyy(i)+tyzzz*quadzz(i)+2.0d0*tyyzz*quadyz(i))

      exxquadi=exxquadi+efact4*(chgquadengij*qjrec)/2.0d0
      eyyquadi=eyyquadi+efact4*(chgquadengij*qjrec)/2.0d0
      ezzquadi=ezzquadi+efact4*(chgquadengij*qjrec)/2.0d0
...
!<--- Parallelization_E
      ...
      exxquadj=-efact5*(chgquad2ji*qirec)*dxx+ &
                efact3*(3.0d0*dxx*quadxx(j)+ &
                 ((dxx+dyy)/2.0d0)*quadyy(j)+((dxx+dzz)/2.0d0)*quadzz(j)+ &
                 3.0d0*dxy*quadxy(j)+3.0d0*dxz*quadxz(j)+dyz*quadyz(j))+ &
                2.0d0*efact3*(dxx*quadxx(j)+dxy*quadxy(j)+dxz*quadxz(j))- &
                2.0d0*efact2*quadxx(j)+ &
                efact6*((chgquad2ji*qirec)*txx+ &
                        (chgquadengji*qirec)*dxx)/2.0d0- &
                efact4*(dx*txijdotthetaij+ &
                        quadxx(j)*dx*txxx+quadyy(j)*dy*txxy+ &
                        quadzz(j)*dz*txxz+quadxy(j)*(dx*txxy+dy*txxx)+ &
                        quadxz(j)*(dx*txxz+dz*txxx)+ &
                        quadyz(j)*(dy*txxz+dz*txxy))- &
                efact*(txxxx*quadxx(j)+ &
                2.0d0*txxxy*quadxy(j)+2.0d0*txxxz*quadxz(j)+ &
                txxyy*quadyy(j)+txxzz*quadzz(j)+2.0d0*txxyz*quadyz(j))
      eyyquadj=-efact5*(chgquad2ji*qirec)*dyy+ &
                efact3*(3.0d0*dyy*quadyy(j)+ &
                 ((dxx+dyy)/2.0d0)*quadxx(j)+((dyy+dzz)/2.0d0)*quadzz(j)+ &
                 3.0d0*dxy*quadxy(j)+3.0d0*dyz*quadyz(j)+dxz*quadxz(j))+ &
                2.0d0*efact3*(dyy*quadyy(j)+dxy*quadxy(j)+dyz*quadyz(j))- &
                2.0d0*efact2*quadyy(j)+ &
                efact6*((chgquad2ji*qirec)*tyy+ &
                        (chgquadengji*qirec)*dyy)/2.0d0- &
                efact4*(dy*tyijdotthetaij+ &
                        quadxx(j)*dx*txyy+quadyy(j)*dy*tyyy+ &
                        quadzz(j)*dz*tyyz+quadxy(j)*(dx*tyyy+dy*txyy)+ &
                        quadxz(j)*(dx*tyyz+dz*txyy)+ &
                        quadyz(j)*(dy*tyyz+dz*tyyy))- &
                efact*(txxyy*quadxx(j)+ &
                2.0d0*txyyy*quadxy(j)+2.0d0*txyyz*quadxz(j)+ &
                tyyyy*quadyy(j)+tyyzz*quadzz(j)+2.0d0*tyyyz*quadyz(j))
      ezzquadj=-efact5*(chgquad2ji*qirec)*dzz+ &
                efact3*(3.0d0*dzz*quadzz(j)+ &
                 ((dxx+dzz)/2.0d0)*quadxx(j)+((dyy+dzz)/2.0d0)*quadyy(j)+ &
                 3.0d0*dxz*quadxz(j)+3.0d0*dyz*quadyz(j)+dxy*quadxy(j))+ &
                2.0d0*efact3*(dzz*quadzz(j)+dxz*quadxz(j)+dyz*quadyz(j))- &
                2.0d0*efact2*quadzz(j)+ &
                efact6*((chgquad2ji*qirec)*tzz+ &
                        (chgquadengji*qirec)*dzz)/2.0d0- &
                efact4*(dz*tzijdotthetaij+ &
                        quadxx(j)*dx*txzz+quadyy(j)*dy*tyzz+ &
                        quadzz(j)*dz*tzzz+quadxy(j)*(dx*tyzz+dy*txzz)+ &
                        quadxz(j)*(dx*tzzz+dz*txzz)+ &
                        quadyz(j)*(dy*tzzz+dz*tyzz))- &
                efact*(txxzz*quadxx(j)+ &
                2.0d0*txyzz*quadxy(j)+2.0d0*txzzz*quadxz(j)+ &
                tyyzz*quadyy(j)+tzzzz*quadzz(j)+2.0d0*tyzzz*quadyz(j))
      exyquadj=-efact5*(chgquad2ji*qirec)*dxy+ &
                efact3*((3.0d0*dxy/2.0d0)*(quadxx(j)+quadyy(j))+ &
                 (dxy/2.0d0)*quadzz(j)+(dxx+dyy)*quadxy(j)+ &
                 dxz*quadyz(j)+dyz*quadxz(j))+ &
                efact3*((dxx+dyy)*quadxy(j)+ &
                        dxy*(quadxx(j)+quadyy(j))+ &
                        dxz*quadyz(j)+dyz*quadxz(j))- &
                2.0d0*efact2*quadxy(j)+ &
                efact6*((chgquad2ji*qirec)*txy+ &
                        (chgquadengji*qirec)*dxy)/2.0d0- &
                efact4*((dx*tyijdotthetaij+ &
                         dy*txijdotthetaij)/2.0d0+ &
                         quadxx(j)*dx*txxy+quadyy(j)*dy*txyy+ &
                         quadzz(j)*dz*txyz+quadxy(j)*(dx*txyy+dy*txxy)+ &
                         quadxz(j)*(dx*txyz+dz*txxy)+ &
                         quadyz(j)*(dy*txyz+dz*txyy))- &
                efact*(txxxy*quadxx(j)+ &
                2.0d0*txxyy*quadxy(j)+2.0d0*txxyz*quadxz(j)+ &
                txyyy*quadyy(j)+txyzz*quadzz(j)+2.0d0*txyyz*quadyz(j))
      exzquadj=-efact5*(chgquad2ji*qirec)*dxz+ &
                efact3*((3.0d0*dxz/2.0d0)*(quadxx(j)+quadzz(j))+ &
                 (dxz/2.0d0)*quadyy(j)+(dxx+dzz)*quadxz(j)+ &
                 dxy*quadyz(j)+dyz*quadxy(j))+ &
                efact3*((dxx+dzz)*quadxz(j)+ &
                        dxz*(quadxx(j)+quadzz(j))+ &
                        dxy*quadyz(j)+dyz*quadxy(j))- &
                2.0d0*efact2*quadxz(j)+ &
                efact6*((chgquad2ji*qirec)*txz+ &
                        (chgquadengji*qirec)*dxz)/2.0d0- &
                efact4*((dx*tzijdotthetaij+ &
                         dz*txijdotthetaij)/2.0d0+ &
                        quadxx(j)*dx*txxz+quadyy(j)*dy*txyz+ &
                        quadzz(j)*dz*txzz+quadxy(j)*(dx*txyz+dy*txxz)+ &
                        quadxz(j)*(dx*txzz+dz*txxz)+ &
                        quadyz(j)*(dy*txzz+dz*txyz))- &
                efact*(txxxz*quadxx(j)+ &
                2.0d0*txxyz*quadxy(j)+2.0d0*txxzz*quadxz(j)+ &
                txyyz*quadyy(j)+txzzz*quadzz(j)+2.0d0*txyzz*quadyz(j))
      eyzquadj=-efact5*(chgquad2ji*qirec)*dyz+ &
                efact3*((3.0d0*dyz/2.0d0)*(quadyy(j)+quadzz(j))+ &
                 (dyz/2.0d0)*quadxx(j)+(dyy+dzz)*quadyz(j)+ &
                 dxy*quadxz(j)+dxz*quadxy(j))+ &
                efact3*((dyy+dzz)*quadyz(j)+ &
                        dyz*(quadyy(j)+quadzz(j))+ &
                        dxy*quadxz(j)+dxz*quadxy(j))- &
                2.0d0*efact2*quadyz(j)+ &
                efact6*((chgquad2ji*qirec)*tyz+ &
                        (chgquadengji*qirec)*dyz)/2.0d0- &
                efact4*((dy*tzijdotthetaij+ &
                         dz*tyijdotthetaij)/2.0d0+ &
                        quadxx(j)*dx*txyz+quadyy(j)*dy*tyyz+ &
                        quadzz(j)*dz*tyzz+quadxy(j)*(dx*tyyz+dy*txyz)+ &
                        quadxz(j)*(dx*tyzz+dz*txyz)+ &
                        quadyz(j)*(dy*tyzz+dz*tyyz))- &
                efact*(txxyz*quadxx(j)+ &
                2.0d0*txyyz*quadxy(j)+2.0d0*txyzz*quadxz(j)+ &
                tyyyz*quadyy(j)+tyzzz*quadzz(j)+2.0d0*tyyzz*quadyz(j))

      exxquadj=exxquadj+efact4*(chgquadengji*qirec)/2.0d0
      eyyquadj=eyyquadj+efact4*(chgquadengji*qirec)/2.0d0
      ezzquadj=ezzquadj+efact4*(chgquadengji*qirec)/2.0d0
