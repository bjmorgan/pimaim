...
!<--- Parallelization_E
!
! This is the muTquad term (AGUADO)
! Force and electric field gradient due to quadrupoles.
!
            temp=-onethird*(ckcnoqall*arijdotcij+cksnoqall*arijdotsij)

!---> Parallelization_S
!           exx=exx+rkx3*rkx3*temp
!           eyy=eyy+rky3*rky3*temp
!           ezz=ezz+rkz3*rkz3*temp
!           exy=exy+rkx3*rky3*temp
!           exz=exz+rkx3*rkz3*temp
!           eyz=eyz+rky3*rkz3*temp
            eltmp(3*num+1:4*num)=eltmp(3*num+1:4*num)+rkx3*rkx3*temp
            eltmp(4*num+1:5*num)=eltmp(4*num+1:5*num)+rky3*rky3*temp
            eltmp(5*num+1:6*num)=eltmp(5*num+1:6*num)+rkz3*rkz3*temp
            eltmp(6*num+1:7*num)=eltmp(6*num+1:7*num)+rkx3*rky3*temp
            eltmp(7*num+1:8*num)=eltmp(7*num+1:8*num)+rkx3*rkz3*temp
            eltmp(8*num+1:9*num)=eltmp(8*num+1:9*num)+rky3*rkz3*temp
!<--- Parallelization_E
         endif
      enddo   
!---> Parallelization_S
